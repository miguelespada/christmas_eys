(($) ->
        ...
        <div>
          <% for(var i=0; i<files.length; i++){ %>
            <div>
              ...
              <% } else { %>
                <img
                  src="<%= $.cloudinary.url(files[i].public_id, { "version": files[i].version, "format": 'jpg', "crop": 'fill', "width": 100}) %>"
                  />
              <% } %>
  ...
  class $.attachinary.Attachinary
    ...
    pushUpFile: (fileToPush) ->
      _files = []
      fileToInsert = null
      i = 0
      pos = 0
      for file in @files
        if file.public_id == fileToPush
          fileToInsert = file
          pos = i - 1
        else
          _files.push file
    ...
    redraw: ->
      ...
      if @files.length > 0
        ...
        @$filesContainer.find('[data-up]').on 'click', (event) =>
          event.preventDefault()
          @pushUpFile $(event.target).data('up')
